...
aggr(fishCounts,
     ...
     numbers = TRUE, 
     sortVars = TRUE,
     labels = names(fishCounts), 
     cex.axis = 0.7,
     gap = 3, 
     ylab = c("Missing Data", "Pattern"), 
     main = "FishCounts")

(list_na <- colnames(fishCounts)[ apply(fishCounts, 2, anyNA)])
```
No missing data
...
fishCounts2 <- Metrics_fish(fishCounts)
```
...
macroCounts2 <- Metrics_macro(macroCounts)
```
...
startDate <- as.POSIXct(ISOdate(startDate, 01, 01, 0, 0, 0, tz = "UTC"))

endDate <- as.numeric(year(min(max(fishCounts2$DateTime),
               max(macroCounts2$DateTime))))

endDate <- as.POSIXct(ISOdate(endDate, 12, 31, 23, 59, 59, tz = "UTC"))
...
###Average data over previous 4 hour period
####Micro data
Sridhar Ravula
```{r}
...
### Calculate Correlaton between MacroCount and Macro metrics
Pranita Patil
```{r}
if (!require('ggplot2')) install.packages('ggplot2', quiet=TRUE)
library(ggplot2)
...
# Calculate Correlation matrix for fish data along with fish metrics
allData4 <- macroCounts2[,-c(1:2, 228)]

corre_ent4 <- rcorr(as.matrix(allData4))

cor_coef4 <- corre_ent4$r
cor_p4 <- corre_ent4$P
...
# Print the heatmap
print(ggheatmap1)
ggsave(ggheatmap1, file=paste0("plot_corr_macro_data", i,".png"), width = 20, height = 20, units = "cm")
...
# Visualize above Correlation matrix in two parts for visibility
rcoeff1 <-(cor_coef4[1:45,181:225])
rpvalue1 <- (cor_p4[1:45,181:225])
ggheatmap(rcoeff1, rpvalue1, 2)

rcoeff2 <-(cor_coef4[46:90,181:225])
rpvalue2 <- (cor_p4[46:90,181:225])
ggheatmap(rcoeff2, rpvalue2, 3)

rcoeff3 <-(cor_coef4[91:135,181:225])
rpvalue3 <- (cor_p4[91:135,181:225])
ggheatmap(rcoeff3, rpvalue3, 4)

rcoeff4 <-(cor_coef4[136:180,181:225])
rpvalue4 <- (cor_p4[136:180,181:225])
ggheatmap(rcoeff4, rpvalue4, 5)

rcoeff5 <-(cor_coef4[181:225,181:225])
rpvalue5 <- (cor_p4[181:225,181:225])
ggheatmap(rcoeff5, rpvalue5, 6)
```
...
for (i in seq(1, dim(cor_coef2)[1], by=28))
  for (j in seq(1, dim(cor_coef2)[1], by=28))
{
    # get whole correlation matrix
    if(i==113 & j!=113){
    upper_tri <- (cor_coef2[i:(i+(dim(cor_coef2)[1]-113)),j:(j+28-1)])
    upper_tri_p <- (cor_p2[i:(i+(dim(cor_coef2)[1]-113)),j:(j+28-1)])  
    }
    else if(j==113 & i!=113){
      upper_tri <- (cor_coef2[i:(i+28-1),j:(j+(dim(cor_coef2)[1]-113))])
      upper_tri_p <- (cor_p2[i:(i+28-1),j:(j+(dim(cor_coef2)[1]-113))]) 
    }
    else if(j==113 & i==113){
      upper_tri <- (cor_coef2[i:(i+(dim(cor_coef2)[1]-113)),j:(j+(dim(cor_coef2)[1]-113))])
      upper_tri_p <- (cor_p2[i:(i+(dim(cor_coef2)[1]-113)),j:(j+(dim(cor_coef2)[1]-113))]) 
    }
    else{
    upper_tri <- (cor_coef2[i:(i+28-1),j:(j+28-1)])
    upper_tri_p <- (cor_p2[i:(i+28-1),j:(j+28-1)])
    }
...
####Clean fish data to just metrics
```{r}
t <- fishCounts2

fishCounts2 <- t %>% dplyr::select(DateTime, 
                                   StationID,
                                   Site.Name, 
                                   pielou, 
                                   speciesCount, 
...
for(station in Station[2:length(Station)]) {
  ...
  #Add station's data to the full data set
  ifelse(nrow(temp) > 0, 
    (fishData[(nrow(fishData)+1):(nrow(fishData) + nrow(temp)),] <- temp[1:nrow(temp),]),
    fishData <- fishData)
}

#rename data frame
fishData$StationID.x <- NULL
fishData$StationID.y <- NULL


rm(chem, fish, temp, station)


##Combine micro df with fish & macro counts df
fishData <- merge(fishData,
                 micro2,
                 by = c("DateTime"),
                 all.x = TRUE)

fishData$`Ammonia-N (mg/l)` <- NULL
fishData$`Nitrite (mg/l)` <- NULL
```

```{r}
describe(fishData[,1:20])
```

```{r}
describe(fishData[,21:40])
```

```{r}
describe(fishData[,41:length(fishData)])
```
...
macroCounts2 <- t %>% dplyr::select(DateTime, 
                                   StationID,
                                   Station.Name, 
                                   pielou_macro, 
                                   speciesCount_macro, 
...
#Combine macro counts & chemistry data
temp <- merge(chem,
              macro,
              by = "DateTime",
              all.y = TRUE)
...
for(station in Station[2:length(Station)]) {
  ...
  #Add station's data to the full data set
  ifelse(nrow(temp) > 0, 
    (macroData[(nrow(macroData)+1):(nrow(macroData) + nrow(temp)),] <- temp[1:nrow(temp),]),
    macroData <- macroData)
}

#rename data frame
macroData$StationID.x <- NULL
macroData$StationID.y <- NULL


rm(chem, macro, temp, station)


##Combine micro df with fish & macro counts df
macroData <- merge(macroData,
                 micro2,
                 by = c("DateTime"),
                 all.x = TRUE)

macroData$`Ammonia-N (mg/l)` <- NULL
macroData$`Nitrite (mg/l)` <- NULL
```

```{r}
describe(macroData[,1:20])
```

```{r}
describe(macroData[,21:40])
```

```{r}
describe(macroData[,41:length(macroData)])
```
